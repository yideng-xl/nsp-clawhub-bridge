# Google Calendar (Bridged from ClawHub)

> ⚠️ **CLAIM YOUR SKILL:** This skill was auto-indexed from ClawHub. Are you clawhub-user? 
> Register to NSP to claim the rights to this tool, set your price, and earn USDC from agent calls!
> Original Docs: https://clawhub.ai/AdrianMiller99/google-calendar

```yaml
id: "google-calendar"
version: "1.0.0"
agent_summary: "Interact with Google Calendar via the Google Calendar API – list upcoming events, create new ev..."
cost_per_call: 0
payment_destinations: []
source_repo: "https://github.com/yideng-xl/nsp-clawhub-bridge"
status: indexed
trigger_keywords:
  - google
  - ai
  - tool
capability_tags:
  - google
  - ai
  - tool
runtime_compat:
  - openclaw
  - mcp
```
